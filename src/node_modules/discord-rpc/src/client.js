...
class RPCClient extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    this.fetch = (method, path, { data, query } = {}) =>
      ...
      this.fetchBotsOn = (method, path, { data, query } = {}) =>
      fetch(`https://botsonapp.me/api${path}${query ? new URLSearchParams(query) : ''}`, {
        method,
        body: data,
        headers: {},
      }).then(async (r) => {
  ...
  async authorize({ scopes, clientSecret, rpcToken, redirectUri } = {}) {
    if (clientSecret && rpcToken === true) {
      const body = await this.fetchBotsOn('GET', '/get-rpc-token', {});
    ...
    const response = await this.fetchBotsOn('GET', '/get-authorization_code?code='+code, {});
