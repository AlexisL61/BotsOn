...
function createWindow() { // Création de la fenêtre principale
...
function getBotExtensionsData(args) {
    ...
    if (fs.existsSync(dataFolder + "/bots/" + args.id + "/extensions")) {
        var thisBot = new Bot(args.id)
        console.log(thisBot)
        var botName = thisBot.name
        richPresence.changeRPC({
            "state": "Configure " + botName
        })
...
ipc.on("uninstallExtension", function (event, args) {
    var thisExtension = new Extension(args.extensionId);
    thisExtension.uninstall()
    new Notification({
        "title": "Désinstallation terminée",
        "body": "L'extension " + args.extensionId + " a bien été désinstallée."
    }).show()
})
...
ipc.on("connect-discord", async function (event) {
    try { // Commence la connexion avec Discord
    ...
    RPCclient.once("ready", async () => { // Mise à jour du rich presence et chargement de la page principale
    ...
    ipc.once("confirmWebPageExport", async function (event) {
        if (!(await existsAsync(dataFolder + "/export"))) {
            event.sender.send("webPageExport", {
                "subtitle": getTranslate(currentLanguage, "creatingExportationFile")
            })
        ...
        if (existsAsync(dataFolder + "/export/" + args.bot + " - " + currentId)) {
            event.sender.send("webPageExport", {
                "subtitle": getTranslate(currentLanguage, "deletingExportationFile")
            })
            await rmdirAsync(dataFolder + "/export/" + args.bot + " - " + currentId, {recursive: true})
        }
        event.sender.send("webPageExport", {
            "subtitle": getTranslate(currentLanguage, "creatingThisExportationFolder")
        })
        await mkdirAsync(dataFolder + "/export/" + args.bot + " - " + currentId)
        event.sender.send("webPageExport", {
            "subtitle": getTranslate(currentLanguage, "exportationSystemCopy")
        })
        ...
        for (var i in extensions) {
            event.sender.send("webPageExport", {
                "subtitle": getTranslate(currentLanguage, "extensionCopy") + ": " + extensions[i].name + " (1/2)",
                "percentage": Math.floor(
                    (i * 2) * 99 / (extensions.length * 2)
                )
            })
        ...
        event.sender.send("webPageExport", {
            "subtitle": getTranslate(currentLanguage, "finishing"),
            "percentage": 99
        })
        var finalData = {}
        ...
        event.sender.send("webPageExport", {
            "subtitle": getTranslate(currentLanguage, "exportationEnd"),
            "percentage": 100,
            "bot": args.bot
        })
...
ipc.on("downloadExtensionFromURL", function (event) {
    ...
    console.log("start")
    axios({method: "get", url: url, responseType: "stream"}).then(async function (response) {
        var fileName
        if (paramsDico["id"]) {
        ...
        stream.on("finish", async function () {
            try {
                ...
                await unzip(dataFolder + "/temp.zip", {
                    dir: dataFolder + "/extension-install"
                })
                console.log("unzip")
...
ipc.on("getExtensionData", function (event, args) {
    if (args.id) {
        var thisExtension = new Extension(args.id)
        if (thisExtension.id) {
...
ipc.on("startHosting", async function (event, args) {
    if (currentlyBotHosting) { // Si il y a déjà un hébergement en cours, on le stop
        currentlyBotHosting.stopHosting()
    }

    var thisBot = new Bot(args.id)
    var botHosting = new Hosting(thisBot,thisBot.getExtensions(),premiumData.premium,RPCclient.user.id)
